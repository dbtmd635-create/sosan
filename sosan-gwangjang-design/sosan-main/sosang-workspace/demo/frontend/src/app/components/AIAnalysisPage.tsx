...
export function AIAnalysisPage() {
  ...
  const handleReset = () => { setFlow("typeSelect"); setAnswers({}); };
  ...
  /* ── 설문 스텝 (신생 창업자 q1: 예/아니오) ── */
  if (flow === "q1") return (
    <div style={{ ...PAGE_BG, padding: "32px 20px" }}>
      <div style={{ maxWidth: "840px", margin: "0 auto" }}>
        ...
        <h2 className="mb-10" style={{ fontSize: "clamp(1.6rem, 4vw, 2.4rem)", fontWeight: 800, letterSpacing: "-0.04em", lineHeight: 1.2 }}>
          창업에 대한 구체적인<br />계획이 있으신가요?
        </h2>

        <div className="grid grid-cols-2 gap-4 mb-10">
          {[
            { label: "예, 계획이 있어요", value: "yes", emoji: "✅" },
            { label: "아직 없어요", value: "no", emoji: "🤔" },
          ].map((opt) => (
            <button
              key={opt.value}
              onClick={() => {
                set("hasPlan", opt.value);
                setFlow("q2");
              }}
              className="flex flex-col items-center justify-center gap-3 transition-all active:scale-[0.97]"
              style={{
                padding: "36px 20px",
                borderRadius: "20px",
                border: ans("hasPlan") === opt.value
                  ? "1.5px solid #10b981"
                  : "1px solid rgba(255,255,255,0.1)",
                background: ans("hasPlan") === opt.value
                  ? "rgba(16,185,129,0.12)"
                  : "rgba(255,255,255,0.04)",
                color: ans("hasPlan") === opt.value ? "#34d399" : "rgba(255,255,255,0.7)",
                fontSize: "1.05rem",
                fontWeight: 600,
                cursor: "pointer",
              }}
